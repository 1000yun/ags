...
docker service update --image registry.cn-hangzhou.aliyuncs.com/ags/image_openresty_dev  ags_openresty >> $LOGFILE  2>&1
if [ "$?" != 0 ] ;
      then    
       echo "err! update  ags_openresty_dev serivce err!"  | tee -a $LOGFILE
       show_log
       exit 0  
     else    
         echo "success!"  | tee -a $LOGFILE
